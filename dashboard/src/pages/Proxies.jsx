...
function GroupForm({ initial, onSave, onClose }) {
    const [form, setForm] = useState({ name: initial?.name || '', description: initial?.description || '', color: initial?.color || '#10b981' })
    ...
    return (
        <form onSubmit={submit} className="space-y-4">
            ...
            <div>
                ...
                <input autoFocus value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} className={inputCls} placeholder="VD: VN-ISP, US-DC, Rotating..." />
            </div>
            <div>
